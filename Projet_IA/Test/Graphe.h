...
/**
 * operateur = obligatoire car la classe comporte une partie dynamique
 * */
...
template <class S, class T>
const Graphe<S,T> & Graphe<S,T>::operator = (const Graphe<S,T> & g)
{
	if(this!=&g)
	{
		int i,j,k;
		...
		nbsommet=g.nombreSommets();
		nbarete=g.nombreAretes();
		Sommet<InfoSommetCarte> * a[6];
		PElement<Sommet<InfoSommetCarte> > *ss;
		for(k=0,ss=g.lSommets;ss;ss=ss->s)
		{
